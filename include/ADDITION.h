...
namespace iRRAM
{
  ...
  public:
    ...
    {
      auto new_node = std::make_shared<ADDITION>(this->lhs->simplify(), this->rhs->simplify());
      return new_node;
  ...
  public:
    ...
    REAL get_M(const REAL& r, const Args&... args) const override {
      return this->node->get_M(r, args...)+abs(this->scalar);
    };
    ...
    std::shared_ptr<Node<R,Args...>> simplify() const override
    {
      ...
      default:
        return std::make_shared<SCALAR_ADDITION>(this->node->simplify(), this->scalar);
      }

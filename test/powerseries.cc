...
void compute(){
  using seq = std::function<std::shared_ptr<POWERSERIES<1,REAL>>(const unsigned long)>;
  using seq_ptr = shared_ptr<seq>;
  function<std::shared_ptr<REAL>(const unsigned long)> series0 = [] (const unsigned long n) {return std::make_shared<REAL>(int(n+1));};
  seq series = [] (const unsigned long n) {
    auto subseq = [n] (unsigned long m) {return std::make_shared<REAL>(int(n+m+1));};
    return std::make_shared<POWERSERIES<1,REAL>>(subseq);
  ...
  auto inv1 = *inverse(std::make_shared<POWERSERIES<1,REAL>>(pwr1));
  ...
  auto deriv=derivative(std::make_shared<POWERSERIES<3,REAL>>(test2), 1, 1);
